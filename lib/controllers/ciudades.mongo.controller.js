...
export async function crearCiudad(req, res) {
    ...
    const nuevaCiudad = new ciudadesModel({ nombre: req.body.nombre, pais: req.body.pais, poblacion: Number(req.body.poblacion) });
    // paso 2 - lo inserto
...
export async function getCiudadesSimple(req, res) {
    ...
    res.json({
        ...
        retocado: ciudadesEnLaBase.map(ciudad => ({...ciudad.toObject(), temperatura: 25}))
    });
};
 ...
 */
export async function getCiudadesLean(req, res) {
    const ciudadesModel = mongoose.model('Ciudad', ciudadSchema);
...
export async function getCiudades(req, res) {
    ...
    const ciudadesConNombreEnEspaniol = datosDeCiudades.map(ciudad => ({
        ...ciudad, nombre: ciudad.nombre.es || ciudad.nombre
    }));
...
export async function getCiudadesTerminado(req, res) {
    ...
    // por qué conviene siempre pasar toObject
    const ciudadesConUnNombre = ciudadesEnLaBase.map(c => ({...c.toObject(), nombre: c.nombre.es || c.nombre }));
